...
export default function DiceRollPage() {
  ...
  const { activeCharacterId, activeRole } = useRPG();
  ...
  const { data: attributesData } = trpc.rpg.attributes.get.useQuery(
    { characterId: activeCharacterId || 0 },
    { enabled: !!activeCharacterId }
  );

  const { data: selectedCharDetails } = trpc.rpg.characters.get.useQuery(
    { id: activeCharacterId || 0 },
    { enabled: !!activeCharacterId }
  );

  // Mestre sempre pode rolar, jogador só se tiver personagem vinculado
  const canRoll = activeRole === "mestre" || (activeRole === "jogador" && !!activeCharacterId);
  ...
  // Filtrar histórico para remover duplicatas (últimas 20 entradas)
  const filteredHistory = diceHistory?.slice(0, 20) || [];

  return (
    <div className="space-y-3 sm:space-y-4">
      {/* Character Display at Top - Only for players with bound character or masters */}
      {selectedCharDetails && (
        <Card className={`${getClassColor(selectedCharDetails.classe).bg} border-2 ${getClassColor(selectedCharDetails.classe).border}`}>
          <CardContent className="pt-4">
            <div className="space-y-3">
              {/* Character Header */}
              <div className="flex items-center justify-between">
                <div>
                  <h3 className="text-lg sm:text-xl font-bold text-amber-500">{selectedCharDetails.name}</h3>
                  <p className="text-xs sm:text-sm text-slate-400">{selectedCharDetails.classe} • Nível {selectedCharDetails.nivel}</p>
                </div>
              </div>
              ...
              <div className="grid grid-cols-2 gap-2 text-xs sm:text-sm">
                <div className="flex items-center gap-2">
                  ...
                  <span className="font-bold text-red-400">{selectedCharDetails.hp}/{selectedCharDetails.hpMax}</span>
                </div>
                <div className="flex items-center gap-2">
                  <span className="text-slate-400">⚡ {selectedCharDetails.vigorType === 'vigor' ? 'Vigor' : 'Mana'}:</span>
                  <span className="font-bold text-blue-400">{selectedCharDetails.vigor}/{selectedCharDetails.vigorMax}</span>
                </div>
              </div>
      ...
      {/* Dice Rolling Section - Show for Master always, for Player only if character bound */}
      {canRoll && (
        <>
          {/* Dice Type Selection */}
      ...
      {/* Message for players without character */}
      {activeRole === "jogador" && !activeCharacterId && (
        <Card className="bg-slate-800 border-slate-700 border-dashed">
          <CardContent className="pt-6 pb-6">
            <p className="text-center text-slate-400 text-sm">
              Vincule um personagem na sala de espera para começar a rolar dados
            </p>
          </CardContent>
        </Card>
      )}
